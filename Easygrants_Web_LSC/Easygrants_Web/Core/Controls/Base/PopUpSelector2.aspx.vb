...
Namespace Core_Web.Controls.Base
	...
	Public MustInherit Class cPopUpSelector2
		Inherits Core.Web.cCorePage

		'DIFFERENCES BETWEEN SUBCLASS AND SHARED CONTROL
		'1. configurable NORESULTSMESSAGE attribute been added.(to be included in shared control) RN 12/19/2005
		'This control is a copy of the shared control. 
		...
		Protected trCancelFind As HtmlTableRow
		...
		Protected spnDisplayLabel As HtmlGenericControl
		Protected spnDisplayLabel1 As HtmlGenericControl
		...
		Protected spRefreshParentPageReturnURL As HtmlGenericControl
		Protected ValSearch As RequiredFieldValidator
		Protected valSearchReq As CustomValidator
		...
		Public mPopUpTitle As String
		Private mReturnURL As String
		...
		Public Property DataObjDisplay() As String
			Get
				If Request.QueryString("DataObjName") = "Person" Or Request.QueryString("DataObjName") = "Persons" Then
					DataObjDisplay = "contact"
				ElseIf Request.QueryString("DataObjName") = "OrganizationName" Then
					DataObjDisplay = GetLabel("Organization")
				ElseIf Request.QueryString("DataObjName") = "PersonUserType" Then
					Return "Person"
				ElseIf Request.QueryString("DataObjName") = "GranteeProjectTitle" Then
					Return "grant record"
				Else
					Return Request.QueryString("DataObjName")
				End If
		...
		Public Overridable Property DataObjDisplay1() As String
			Get
				If Request.QueryString("DataObjName") = "Person" Or Request.QueryString("DataObjName") = "Persons" Or Request.QueryString("DataObjName") = "StaffUserType" Then
					DataObjDisplay1 = "Contact"
				ElseIf Request.QueryString("DataObjName") = "PersonOrganization" Then
					DataObjDisplay1 = "Contact"
				ElseIf Request.QueryString("DataObjName") = "OrganizationName" Then
					DataObjDisplay1 = GetLabel("Organization")
				ElseIf Request.QueryString("DataObjName") = "Organization" Then
					DataObjDisplay1 = GetLabel("Organization")
				ElseIf Request.QueryString("DataObjName") = "PersonUserType" Then
					Return "Person"
				ElseIf Request.QueryString("DataObjName") = "GranteeProjectTitle" Then
	   ...
	   '----------------------------------------------------------------------
		...
		Public ReadOnly Property QueryString() As String
			Get
				If Request.QueryString("ReturnURLQueryString") = "" Then
					If Request.QueryString("ValueQueryString") <> "" Then
						QueryString = Me.Request.QueryString("ValueQueryString")
					End If
				Else
					If Request.QueryString("TextQueryString") <> "" Then
						QueryString = Me.Request.QueryString("TextQueryString")
					Else
						QueryString = Me.Request.QueryString("ValueQueryString")
					End If
				End If
		...
		Public Property PopUpTitle() As String
			Get
				If DataObjName = "Person" Or DataObjName = "Persons" Or DataObjName = "PersonUserType" Then
					PopUpTitle = "Contact Selector"
				ElseIf DataObjName = "Organization" Or DataObjName = "OrganizationName" Then
					PopUpTitle = GetLabel("Organization") + " Selector"
				ElseIf DataObjName = "GranteeProjectTitle" Then
					PopUpTitle = "Grant Selector"
				Else
		...
		Public Overrides Sub CorePage_Load(ByVal aSrc As Object, ByVal aEvent As System.EventArgs)
			...
			If aAlternateText = "" Then
				If DisplayName = "Organization name" Then
					spnText.InnerHtml = "Enter all or part of the " + "organization common name or EIN" + " in the text box below. Then, click <b>Find</b> to find the organization. "
				ElseIf DisplayName = "Organization Name" Then
					spnText.InnerHtml = "Enter all or part of the organization name in the text box below. Click <b>Find</b> to find the organization. "

				Else
					spnText.InnerHtml = "Enter all or part of the " + DisplayName + " in the text box below. Then, click <b>Find</b>. "
				End If
			...
			aSizeValue = cXMLDoc.AttributeToInt(aXmlNode, "Size", 0)

			aConfigXml.Append(aSearchValue)
			If aSizeValue <> 0 Then
				txtSearch.Size = aSizeValue
			End If
			aConfigXml.Append("'/>")
			...
			spnDisplayLabel.InnerHtml = ConfigurableText(cXMLDoc.AttributeToString(aXmlNode, "DisplayLabel"))
			...
			If Not Request.QueryString("find") Is Nothing Then
				...
				If DisplayName = "Organization Name" Then
					spnDisplayLabel1.InnerHtml = "If the organization does not appear in the list, click <b>Cancel</b> and click the <b>Add New Organization</b> link to add a new organization to Easygrants."
				End If
				spnDisplayLabel1.InnerHtml = ConfigurableText(spnDisplayLabel1.InnerHtml)
				...
				If ctlInstitutionList.Items.Count < 1 Then
					...
					If aNoResultsMessage = "" Then
						spnNoResultsMessage.InnerHtml = "<font color='red'>No results were found. Click <b>Cancel</b> to start a new search.</font>"
					Else
						spnNoResultsMessage.InnerHtml = "<font color='red'>" + ConfigurableText(aNoResultsMessage) + "</font>"
					End If
				End If
			End If
		End Sub
		...
		Public Sub btnSelect_Click(ByVal aSrc As Object, ByVal aArgs As EventArgs)
			...
			If Request.QueryString("RefreshParentPage") = "True" Then
			   ...
			   ' Dim aParentPageURL as String = Request.Url.PathAndQuery

				'if Request.QueryString("ReturnURLQueryString") = "True" Then
					spRefreshParentPageReturnURL.Visible = True
				'else
		...
		Public Sub ValidateLength(ByVal aSrc As Object, ByVal aArgs As ServerValidateEventArgs)
			...
			'ValSearch.Enabled = True
			'ValSearch.Validate()
			'If Not ValSearch.IsValid Then
		...
		'=============================================================	
		Public Sub ValidateRequired(ByVal aSrc As Object, ByVal aArgs As ServerValidateEventArgs)
			Dim blnGenderSelected As Boolean = False
			'txtSearch = CType(FindControl("ctlGender"), HtmlInputText)
