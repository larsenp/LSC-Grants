...
<ValidationPropertyAttribute("SelectedValueList")> Public MustInherit Class cDoubleListBox2
    Inherits Core.Web.cCoreCtl

	Protected valMaxSelected As CustomValidator
	Protected valMinSelected As CustomValidator
    ...
    Protected tblSort As HtmlTable
        ...
        Protected hdnTextList As HtmlInputHidden
	...
	Private mMaxSelected As Integer = 0
    Private mMinSelected As Integer = 0
    Private mTooManyMessage As String
	Private mTooFewMessage As String

    
'--------------------------------------------------------------

    Public Property MaxSelected() As Integer
        Get
            return mMaxSelected
        End Get
...
'--------------------------------------------------------------
    ...
    Public Property MinSelected() As Integer
        Get
            return mMinSelected
        End Get
        Set(ByVal Value As Integer)
...
'-------------------------------------------------------------
    ...
    Public Property TooManyMessage() As String
        Get
            Return mTooManyMessage
        End Get
        Set(ByVal Value As String)
            mTooManyMessage = Value
        End Set
    End Property

'-------------------------------------------------------------

    Public Property TooFewMessage() As String
        Get
            Return mTooFewMessage
        End Get
        Set(ByVal Value As String)
            mTooFewMessage = Value
        End Set
    End Property
...
'--------------------------------------------------------------
    ...
    End Property
        ...
        Public Property SelectedTextList() As String
            Get
                SelectedTextList = hdnTextList.Value
            End Get
            Set(ByVal Value As String)
                hdnTextList.Value = Value
            End Set
        End Property
...
'----------------------------------------------------------------------------
   ...
   Public Sub ValidateMaxSelected(ByVal aSrc As Object, ByVal aArgs As ServerValidateEventArgs)
		if (MaxSelected <> 0) and SelectedValueList.Split(",").Length > MaxSelected then
			aArgs.IsValid = False
			
			if TooManyMessage <> "" then valMaxSelected.ErrorMessage = TooManyMessage
		end if
   End Sub

'----------------------------------------------------------------------------

   Public Sub ValidateMinSelected(ByVal aSrc As Object, ByVal aArgs As ServerValidateEventArgs)
		if (MinSelected <> 0 and SelectedValueList.Split(",").Length < MinSelected) or _
			(MinSelected = 1 and SelectedValueList.Length = 0) then
			aArgs.IsValid = False
			
			if TooFewMessage <> "" then valMinSelected.ErrorMessage = TooFewMessage
		end if
   End Sub
...
'--------------------------------------------------------------
   ...
   Public Overrides Sub CoreCtl_Configure(ByVal aDisplayPropertyNode As XmlNode, ByVal aDataObjectList As cDataObjectList)
		...
		MaxSelected = cXMLDoc.AttributeToInt(aControlNode, "MaxSelected")
        MinSelected = cXMLDoc.AttributeToInt(aControlNode, "MinSelected")
        TooManyMessage = cXMLDoc.AttributeToString(aControlNode,"TooManyMessage")
        TooFewMessage = cXMLDoc.AttributeToString(aControlNode,"TooFewMessage")
...
'--------------------------------------------------------------
	...
	Public Sub AddItemRight(ByVal aListItem As ListItem)
		...
		If hdnList.Value.ToString = "" Then
                ...
                hdnTextList.Value = aListItem.Text
		Else
			...
			If Array.IndexOf(aSelectedItems, aListItem.Value) < 0 Then
                    ...
                    hdnTextList.Value = hdnTextList.Value & "," & aListItem.Text
			End If
		End If
	End Sub
...
'--------------------------------------------------------------
	...
	Public Overrides Sub CoreCtl_SetDataObjectValueFromCtl(ByVal aDisplayPropertyNode As XmlNode, ByVal aDataObject As cDataObject)
		...
		Dim aRelatedDataObjectNode As XmlNode
		Dim aDefaultValue As String
		dim aSave as Boolean
		...
		aRelatedDataObjectNode = aControlNode.SelectSingleNode("RelatedDataObjects")
		...
		If SelectedValueList <> "" Then
			...
			For Each aItem In aSelectedItems
				aSave = True
				for each aObj in aDataObjectList
					if aObj.GetPropertyString(aStoredValuePropertyKey) = aItem then
						aObj.Checked = True
						aSave = False
						exit For
					End If					
				...
				if aSave = True then
					...
					aChildDataObject = me.WebSession.DataObjectFactory.CreateNewDataObjectFromKey(aDefinitionKey)
					...
					'set stored value
					aChildDataObject(aStoredValuePropertyKey) = aItem
					aChildDataObject.SaveData()
				end if
			Next 
			'save data
			...
			for each aObj in aDataObjectList
				if aObj.Checked = False
					if not aRelatedDataObjectNode is nothing then
						me.EventController.EventController_Delete(aRelatedDataObjectNode, _
							aObj.DefinitionKey, aObj.GetPropertyString(aObj.PrimaryKey), GetUserID())
					End If
					
					me.WebSession.DataObjectFactory.DeleteData(aObj)
				End If					
			Next
		else
			for each aObj in aDataObjectList
	...
	Public Sub UpdateSelectionFromSelectedValueList
		...
		For Each aSelectedValue In aSelectedValues
			...
			If Not aLeftItem Is Nothing AndAlso aLeftItem.Text.Trim <> "" Then
                ...
                'If Not selRight.Items.Contains(aRightItem) Then
                '    selRight.Items.Add(aRightItem)
                'End If
                'selLeft.Items.Remove(aLeftItem)
                End If
            Next
	End Sub
