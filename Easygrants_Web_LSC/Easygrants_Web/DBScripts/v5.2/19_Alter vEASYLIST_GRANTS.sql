...
GO
/****** Object:  View [dbo].[vEASYLIST_GRANTS]    Script Date: 09/08/2008 12:26:56 ******/
SET ANSI_NULLS ON
GO
...
SELECT DISTINCT 
                      ...
                      dbo.PERSON_ORGANIZATION_AFFILIATION.institutional_component,
                          (SELECT     email_addr
                            FROM          dbo.EMAIL AS E
                            WHERE      (is_primary = 1) AND (entity_id = dbo.WF_TASK_ASSIGNMENT.person_id) AND (entity_type_id = 1)) AS primary_email_addr, 
                      dbo.a_COMPETITION.program_id, dbo.a_WF_PROJECT.competition_id, dbo.GRANTEE_PROJECT.wf_project_id, 
                      dbo.GRANTEE_PROJECT.grant_status_id,
                      ...
                      PrimaryStaff.name_last + ', ' + PrimaryStaff.name_first AS PrimaryLastNameFirstName,PrimaryStaff.person_id AS PrimaryPersonID, dbo.GRANTEE_PROJECT.grant_title AS ProjectTitle, 
                      dbo.GRANTEE_PROJECT.grant_description AS ProjectDesc, dbo.GRANTEE_PROJECT.legacy_grant_id, dbo.a_FUND.fund_id, 
                      dbo.a_FUND.fund_name
