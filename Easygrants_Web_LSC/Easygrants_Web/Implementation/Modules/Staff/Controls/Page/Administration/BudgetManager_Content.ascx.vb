...
Public MustInherit Class cBudgetManager_Content
...
'-------------------------------------------------------------
	...
	Public Sub AddBudgetCategory(ByVal aEditorPageKey As String, ByVal BudgetCategoryID As String, ByVal WfProjectID As String, ByVal aReturnURL As String)

		Dim aBudgetCategoryDOList As cDataObjectList = WebSession.DataObjectFactory.GetDataObjectListWithFilter("BudgetCategory", "ProjectID", ProjectID)
		If aBudgetCategoryDOList.Count = 0 Then
			Dim aBudgetCategoryDO As cDataObject = WebSession.DataObjectFactory.CreateNewDataObjectFromKey("BudgetCategory")
			aBudgetCategoryDO("ProjectID") = ProjectID
			aBudgetCategoryDO("ParentID") = 0	'This value will be Zero because we are adding the first Budget category called "Total". which needs to be added before adding the first Budget Category.
			aBudgetCategoryDO("Description") = "Total"
			aBudgetCategoryDO("HasChild") = 1
			aBudgetCategoryDO("HasNotes") = 0
			aBudgetCategoryDO("SortOrder") = 0

			aBudgetCategoryDO.SaveData()
		End If

		EventController_NavigateToModulePageKey(aEditorPageKey, BudgetCategoryID, WfProjectID, aReturnURL)
