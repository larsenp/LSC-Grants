...
Public MustInherit Class cAdHocQueryEditor
...
'========================================================
	...
	Protected btnAddGroup As cCSSButton
	...
	Protected btnSaveAs As cCSSButton
...
'========================================================
    ...
    Public Overrides Sub CoreCtl_Load(ByVal aSrc As Object, ByVal aEvent As System.EventArgs)
		...
		mQueryToolDefinitionXMLDoc = WebAppl.DataDefinitionDoc 'New cXMLDoc(WebAppl.Build_RootFileSystemPath("/BaselineDD/DataDefinition.config"))
        'END TEMPORARY HACK
       ...
       If Not Request.QueryString("QueryType") = "New" Then
			If SavedQueryDO.GetPropertyInt("AccessTypeID", 0) = constPrivate And SavedQueryDO.GetPropertyInt("UserID", 0) <> CType(CType(Page, cModulePage).ModuleUser, cEGStaffUser).UserID Then
...
'--------------------------------------------------------
    ...
    Public Sub ConfigureQueryGroupDropDown()
		...
		Dim aDataObjectNode As XmlNode
            ...
            ' aDOL = WebSession.DataObjectFactory.GetDataObjectListWithFilter("QueryType", "EndDate", "")
            aDOL = Me.DataObjectCollection("QueryType")
...
'--------------------------------------------------------
    ...
    Public Sub btnSave_Click(ByVal aSrc As Object, ByVal aEvent As System.EventArgs)
        WriteSortNode()
        WriteDisplayPropertiesNode()
        ...
        valCusSave.Validate()
        If Not (valReqSelect.IsValid And valReqQueryName.IsValid And valCusDistinctSort.IsValid _
					And valCusSave.IsValid) Then Exit Sub
...
'--------------------------------------------------------
    ...
    Public Sub btnSaveAs_Click(ByVal aSrc As Object, ByVal aEvent As System.EventArgs)
		...
		aNewQuery("QueryTypeID") = SavedQueryDO.GetPropertyInt("QueryTypeID")
		aNewQuery("AccessTypeID") = SavedQueryDO.GetPropertyInt("AccessTypeID")
...
'--------------------------------------------------------
	...
	Public Sub WriteGroupTableRow()
		...
		If Not aSelectedValue = "" Then
			...
			If Not ctlGroupFields.Items.FindByValue(aSelectedValue) Is Nothing Then
...
'--------------------------------------------------------
	...
	Public Sub WriteDisplayPropertiesNode()
		...
		If spnSelect.Visible Then
			...
			If Request.Form.Get("mPageFrameCtl:ctlPageContent:ctlSelect:hdnList") <> "" Then
...
'--------------------------------------------------------
	...
	Public Sub WriteFilterArgumentNode(ByRef aArgumentEl As XmlElement)
		...
		If aIndex <> -1 Then
			aRelatedPropertyKey = aPropertyKey.Substring(0, aIndex)
			aPropertyKey = aPropertyKey.Substring(aIndex + 1)
			aRelatedPropertyEl = aArgumentEl.SelectSingleNode("RelatedProperty")
			If aRelatedPropertyEl Is Nothing Then
				aRelatedPropertyEl = AdHocQueryDefXMLDoc.XMLDocument.CreateElement("RelatedProperty")
