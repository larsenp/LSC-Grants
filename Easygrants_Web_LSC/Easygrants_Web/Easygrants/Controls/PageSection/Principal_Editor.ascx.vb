...
Public Class cPrincipal_Editor
...
'-------------------------------------------------------------
  ...
  '      if ctlIsPrimary.Value = True then
        ...
        if aObjList(0).GetPropertyInt("PersonID", 0) = 0 then
			...
			if aEmailList.Count = 0 then
				...
				if aObjList(0).GetPropertyString("Email") <> "" then
					...
					aEmail("ContactValue1") = aObjList(0).GetPropertyString("Email")
