/****** Object:  StoredProcedure [dbo].[spLSC_TIG_COPY_PAYMENT_SCHEDULE_TO_GRANT]    Script Date: 09/01/2010 13:53:55 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[spLSC_TIG_COPY_PAYMENT_SCHEDULE_TO_GRANT]
	(@wfta_id int)
...
SELECT wfta.grantee_project_id,
	...
	WHERE pps.wf_task_assignment_id = @app_wfta_id
...
INSERT PAYMENT_CONTINGENCY_TASKS
	...
	FROM #wfta
		JOIN PAYMENT p ON #wfta.grantee_project_id = p.grantee_project_id
			AND #wfta.payment_number = p.payment_number
		JOIN WF_TASK_ASSIGNMENT wfta ON wfta.wf_task_assignment_id = @app_wfta_id
