...
SELECT wfta.wf_task_assignment_id,
	...
	wfta.workgroup_id,
	wg.description work_group,
	CASE WHEN pnr.LastFirstMiddle IS NOT NULL THEN pnr.LastFirstMiddle
		WHEN wg.description IS NOT NULL THEN wg.description + ' Workgroup'
		ELSE NULL END reviewer_or_work_group
	FROM WF_TASK_ASSIGNMENT wfta
		JOIN a_WF_TASK wft ON wfta.wf_task_id = wft.id
		LEFT JOIN PERSON pr ON wfta.person_id = pr.person_id
		LEFT JOIN vPERSON_NAMES pnr ON wfta.person_id = pnr.person_id
		...
		LEFT JOIN a_WORK_GROUP wg ON wfta.workgroup_id = wg.id
