/****** Object:  View [dbo].[vTO_DO_LIST_REVIEW]    Script Date: 08/24/2011 17:00:00 ******/
SET ANSI_NULLS ON
GO
...
SELECT wfta.wf_task_assignment_id,
	...
	CASE WHEN wfta.wf_task_id = 72 THEN '' ELSE c.abbr END serv_area,
	CASE WHEN wfta.wf_task_id = 72 THEN '' ELSE wfp.description END funding_cycle,
	...
	WHERE wfta.wf_task_status_id <> 3
		...
		AND c.program_id = 1
        --AND (wft.id < 48 or wft.id = 72 or wft.id = 68 or wft.id = 69 or wft.id = 74 or wft.id = 75)
        --AND (wft.id < 48 or wft.id in (72,68,69, 74, 75))
         AND (wft.id < 47 or wft.id in (72,68,69))
