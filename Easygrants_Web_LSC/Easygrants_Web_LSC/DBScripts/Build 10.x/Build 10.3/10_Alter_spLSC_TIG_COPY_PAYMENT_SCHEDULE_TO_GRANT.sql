--Remove functionality for creating milestones from Project Stage data in application.
--Application now creates multiple milestones per payment.
--Mantis 2841

/****** Object:  StoredProcedure [dbo].[spLSC_TIG_COPY_PAYMENT_SCHEDULE_TO_GRANT]    Script Date: 04/27/2011 09:55:39 ******/
SET ANSI_NULLS ON
GO
...
--Delete payment schedule items not represented in #pps (payments deleted in task assignment)
DELETE LSC_PAYMENT 
	FROM LSC_PAYMENT lp
		JOIN PAYMENT p ON lp.payment_id = p.payment_id
		LEFT JOIN #pps ON lp.lsc_payment_id = #pps.lsc_payment_id
	WHERE p.grantee_project_id = @grantee_project_id
		AND #pps.lsc_payment_id IS NULL

DELETE PAYMENT
	FROM PAYMENT p
		LEFT JOIN #pps ON p.payment_id = #pps.payment_id
	WHERE p.grantee_project_id = @grantee_project_id
		AND #pps.lsc_payment_id IS NULL

DELETE PAYMENT_CONTINGENCY_TASKS
	FROM PAYMENT_CONTINGENCY_TASKS pct
		JOIN WF_TASK_ASSIGNMENT wfta ON pct.contingency_task_assignment_id = wfta.wf_task_assignment_id
		LEFT JOIN #wfta ON pct.payment_contingency_id = #wfta.payment_contingency_id
	WHERE wfta.grantee_project_id = @grantee_project_id
		AND wfta.wf_task_id = 30
		AND #wfta.payment_contingency_id IS NULL

DELETE WF_TASK_ASSIGNMENT 
	FROM WF_TASK_ASSIGNMENT wfta
		LEFT JOIN #wfta ON wfta.wf_task_assignment_id = #wfta.wf_task_assignment_id
	WHERE wfta.grantee_project_id = @grantee_project_id
		AND wfta.wf_task_id = 30
		AND #wfta.wf_task_assignment_id IS NULL
