--Rework task assignments so that actual and projected forms are assigned to the grants corresponding to the years for which data are reported.

/****** Object:  StoredProcedure [dbo].[spCREATE_GAR_TASK_ASSIGNMENTS]    Script Date: 01/05/2011 11:37:08 ******/
SET ANSI_NULLS ON
GO
...
create table #grants
    (organization_id int,
    grantee_project_id_curr int,
    grantee_project_id_prev int,
    person_id int,
    wf_project_id_curr int,
    wf_project_id_prev int,
...
insert #grants
    (organization_id,
    grantee_project_id_curr,
    grantee_project_id_prev,
    person_id,
    wf_project_id_curr,
    wf_project_id_prev,
    service_area,
...
update #grants
	set grantee_project_id_prev = null,
		wf_project_id_prev = null
	from #grants g
		join grantee_project_info gpi on g.grantee_project_id_curr = gpi.grantee_project_id
	where year(gpi.duration_start) > @report_year

update #grants
	set grantee_project_id_prev = gp.grantee_project_id,
		wf_project_id_prev = gp.wf_project_id
	from #grants g
		join a_competition c on g.service_area = c.description
		join a_wf_project wfp on c.id = wfp.competition_id
		join grantee_project gp on wfp.id = gp.wf_project_id
		join grantee_project_info gpi on gp.grantee_project_id = gpi.grantee_project_id
	where g.grantee_project_id_prev is null
		and year(gpi.duration_end) = @report_year
...
declare g cursor for
    select grantee_project_id_curr,
        organization_id
        from #grants
...
begin
    if @organization_id <> @prev_org_id
        update #grants
            ...
            where grantee_project_id_curr = @grantee_project_id
...
insert #gar
    ...
    select wft.id,
        1,
        case when lwft.is_projected = 0 then g.grantee_project_id_prev else g.grantee_project_id_curr end,
        case when lwft.is_projected = 0 then g.wf_project_id_prev else g.wf_project_id_curr end,
        g.person_id,
        8,
		@open_date ,
		@close_date ,
