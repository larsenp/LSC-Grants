...
ALTER PROCEDURE [dbo].[spLSC_ASSIGN_PBIF_RISK_ASSESSMENT_REVIEW]
...
BEGIN
	...
	SELECT @secondary_review_complete = COUNT(*)
	FROM WF_TASK_ASSIGNMENT
	WHERE grantee_project_id = @grantee_project_id 
		AND wf_task_id = 107
		AND wf_task_status_id = 2

   IF @primary_review_status = 2 and @secondary_review_complete >= 1
