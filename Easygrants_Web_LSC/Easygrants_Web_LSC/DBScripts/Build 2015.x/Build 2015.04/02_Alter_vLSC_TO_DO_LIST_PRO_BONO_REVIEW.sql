...
GO
ALTER VIEW [dbo].[vLSC_TO_DO_LIST_PRO_BONO_REVIEW]

AS

WITH base (	wf_task_assignment_id,
	wf_task_id ,
	wf_task_status_id ,
	...
	close_date,
	end_date )
	AS (SELECT MIN(wfta.wf_task_assignment_id) wf_task_assignment_id,
			wfta.wf_task_id,
...
SELECT CAST(base.wf_task_id AS VARCHAR(3)) + '_' + CAST(ISNULL(base.review_cycle_stage_id,0) AS VARCHAR(3)) + '_' + CAST(ISNULL(base.person_id, 0) AS VARCHAR(5)) lsc_to_do_list_pro_bono_review_key,
	base.wf_task_assignment_id,
	base.wf_task_id,
	...
	FROM base
		...
		JOIN a_WF_TASK_MODULE wftm ON base.wf_task_id = wftm.wf_task_id
			AND base.wf_task_role_id = wftm.wf_task_role_id
