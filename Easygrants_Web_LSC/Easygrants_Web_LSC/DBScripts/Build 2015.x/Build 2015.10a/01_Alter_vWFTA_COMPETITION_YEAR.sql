...
select wfta.wf_task_assignment_id, 
	...
	where wfta.wf_task_id in (1, 2, 66)
		...
		or c.program_id = 6
		or (wfta.wf_task_id = 102 and wfta.award_term_id is null)
...
select wfta.wf_task_assignment_id, 
	...
	where c.program_id = 1 
		and (wfta.wf_task_id in (4,7,67,91) 
				or wft.wf_task_type_id = 3
				or (wfta.wf_task_id = 102 and wfta.award_term_id is not null))
...
SELECT wfta.wf_task_assignment_id,
     ...
     FROM grantee_project gp
         ...
         JOIN c_time_period tp ON Year(wftas.start_date) = tp.abbr
		 JOIN a_wf_task wft ON wfta.wf_task_id = wft.id
     WHERE c.program_id = 3 and wft.id in (64, 65, 73, 84, 85, 86)
...
SELECT wfta.wf_task_assignment_id,
     ...
     FROM grantee_project gp
         JOIN wf_task_assignment wfta ON wfta.grantee_project_id = gp.grantee_project_id 
           LEFT JOIN wf_task_assignment wfta2 ON wfta2.grantee_project_id = gp.grantee_project_id and wfta2.wf_task_id = 101
           LEFT JOIN LSC_WFTA_SUBGRANT wftas ON wftas.wf_task_assignment_id = wfta2.wf_task_assignment_id
         JOIN a_wf_project wfp ON gp.wf_project_id = wfp.id
		 JOIN a_competition c ON wfp.competition_id = c.id
         JOIN c_time_period tp ON Year(wftas.start_date) = tp.abbr
		 JOIN a_wf_task wft ON wfta.wf_task_id = wft.id
     WHERE c.program_id = 3 and wft.id in (104, 105, 108, 109)
...
SELECT wfta.wf_task_assignment_id,
     ...
     FROM grantee_project gp
         JOIN wf_task_assignment wfta ON wfta.grantee_project_id = gp.grantee_project_id 
           LEFT JOIN wf_task_assignment wfta2 ON wfta2.grantee_project_id = gp.grantee_project_id and wfta2.wf_task_id = 116
           LEFT JOIN LSC_WFTA_SUBGRANT wftas ON wftas.wf_task_assignment_id = wfta2.wf_task_assignment_id
         JOIN a_wf_project wfp ON gp.wf_project_id = wfp.id
		 JOIN a_competition c ON wfp.competition_id = c.id
         JOIN c_time_period tp ON Year(wftas.start_date) = tp.abbr
		 JOIN a_wf_task wft ON wfta.wf_task_id = wft.id
     WHERE c.program_id = 3 and wft.id in (117, 118, 119, 120, 121)
