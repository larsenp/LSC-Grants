CREATE VIEW [dbo].[vLSC_COMP_APPL_FISCAL_SCORE_BY_PART_2016]
...
SELECT lsc_comp_appl_fiscal_id
	...
	,lsc_deficit_status 
		...
		+ CASE WHEN rcaos.cert_good_standing_meet_req = 1 THEN 1 ELSE 0 END subtotal_part_v --1 point in part V if reviewer finds that the Certificate of Good Standing meets requirements
	,exp_fraud 
		+ exp_misapprop
		+ exp_embezzlement
		+ exp_theft
		+ lsc_fraud_insurance_policy_limit 
		+ CASE WHEN rcaos.fidelity_bond_meet_req = 1 THEN 1 ELSE 0 END subtotal_part_vi --1 point in part VI if reviewer finds that the Current Fidelity Bond Coverage meets requirements
	,sw_track_fund_source 
		+ fiscal_sw_modules_installed 
		+ sw_own_password 
		+ network_security 
		+ lsc_fin_data_backup_interval subtotal_part_vii 
	FROM vLSC_COMP_APPL_FISCAL_SCORE_2016 cafs
		LEFT JOIN LSC_REVIEW_COMP_APPL_OCE_STAFF rcaos ON cafs.oce_staff_review_wfta_id = rcaos.wf_task_assignment_id
