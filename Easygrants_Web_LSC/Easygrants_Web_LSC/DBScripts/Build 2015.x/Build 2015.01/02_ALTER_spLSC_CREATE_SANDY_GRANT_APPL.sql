...
/****** Object:  StoredProcedure [dbo].[spLSC_CREATE_SANDY_GRANT_APPL]    Script Date: 1/26/2015 3:37:09 PM ******/
SET ANSI_NULLS ON
GO
...
-- Assign grant number if it hasn't already been assigned
EXEC spLSC_ASSIGN_PBIF_NUMBER
	@grantee_project_id = @new_gp_id
...
--Altum chose to use this SP for creation of Pro Bono grants, which take their due and close dates from the funding cycle.
IF (@wf_task_id = 100  or  @wf_task_id = 123)
	UPDATE WF_TASK_ASSIGNMENT
		SET due_date = NULL
			,close_date = NULL
		WHERE wf_task_assignment_id = @new_wfta_id
