...
create table #csr
    ...
    Cases1YrAgo int,
	Cases2YrsAgo int,
    PAICasesCurrent int,  --new in 2014 cycle
    PAICases1YrAgo int,     -- new in 2015 cycle
    PAICases2YrsAgo int,    -- new in 2015 cycle
	ExtendedCases int,
    ContestedCases int,   --new in 2014 cycle
	PovertyPopulation int,
	CasesPer10k decimal(10,3),
    PAICasesPer10k decimal(10,3),   --new in 2014 cycle
    ...
    CasesDiffPercent int,
    CasesPctIncrease int,
    PAICasesDiffPercent int,
    PAICasesPctIncrease int,
    CasesDecreasePrevYr_20  bit default 0,
    PAICasesDecreasePrevYr_20  bit default 0,
    PAICSRBelow70 bit default 0,
	StaffFTEsCurrent decimal(6,2) default 0.0,
	StaffFTEs1YrAgo decimal(6,2) default 0.0,
	StaffFTEs2YrsAgo decimal(6,2) default 0.0,
	StaffFTEsPctIncrease int,
	OtherFunding1YrAgo int,
	OtherFunding2YrsAgo int,
	OtherFundingPctIncrease int)
...
--Get PAI case totals for the previous year and write them to #csr.PAIcases_1_yr_ago
...
--Get PAI case totals for two years prior and write them to #csr.PAIcases2yrsago
...
--Revenues. Revenues are collected for basic field only. Carryover is excluded.
declare @amount int
...
update #csr
	set OtherFunding1YrAgo = @amount

update #csr
	set OtherFundingPctIncrease = 100*(OtherFunding - OtherFunding1YrAgo)/OtherFunding1YrAgo
	where OtherFunding1YrAgo is not null and OtherFunding1YrAgo <> 0
...
--Staff FTEs	
update #csr
	set StaffFTEsCurrent = staff.fte
	from (
		select recipient_id, sum(fte) fte 
			from vLSC_DP_STAFF_DETAIL
			where recipient_id = @recipient_id
				and year = @year - 2
				and include_report = 1
			group by recipient_id
		) staff

update #csr
	set StaffFTEs1YrAgo = staff.fte
	from (
		select recipient_id, sum(fte) fte 
			from vLSC_DP_STAFF_DETAIL
			where recipient_id = @recipient_id
...
update #csr
	set StaffFTEsPctIncrease = 100*(StaffFTEsCurrent - StaffFTEs1YrAgo)/StaffFTEs1YrAgo
	where StaffFTEs1YrAgo is not null and StaffFTEs1YrAgo <> 0

update #csr
	set StaffFTEs2YrsAgo = staff.fte
	from (
		select recipient_id, sum(fte) fte 
			from vLSC_DP_STAFF_DETAIL
			where recipient_id = @recipient_id
...
update #csr
	set PAICSRBelow70 = 1
	where PAICasesPer10k < MedianPAICasesPer10k_70
...
update #csr
	set CasesDiffPercent = 100.0*((Cases1YrAgo - CasesCurrent) / CAST(Cases1YrAgo AS DECIMAL))
	where Cases1YrAgo <> 0

update #csr
	set PAICasesDiffPercent = 100.0*((PAICases1YrAgo - PAICasesCurrent) / CAST(PAICases1YrAgo AS DECIMAL))
	where PAICases1YrAgo <> 0

--The 'DiffPercent' columns were originally built to calculate decreases in measures. Later, a requirement arose to
--display percent changes, which need to be expressed as increases.
update #csr
	set CasesPctIncrease = -1*CasesDiffPercent
		,PAICasesPctIncrease = -1*PAICasesDiffPercent

Update #csr
    set CasesDecreasePrevYr_20 = 1
    where CasesDiffPercent > 20

Update #csr
    set PAICasesDecreasePrevYr_20 = 1
    where PAICasesDiffPercent > 20
