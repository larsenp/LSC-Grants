ALTER PROCEDURE [dbo].[spLSC_ASSIGN_GAR_FORMS]
	(@report_year int,
	@open_date datetime,
    ...
    @subgrant_duration_start datetime,
   @subgrant_duration_end datetime*/)
...
insert #gar
    ...
    select wft.id,
        ...
        where wft.wf_task_type_id = 3
			...
			and lwft.lsc_gar_form_group_id = @lsc_gar_form_group_id
            and wft.id != 27  -- excluded the E-1(c) forms

--Populate temp table with one task assignment for every combination of active grant and GAR task
--for E-1(c)forms assigned only to 2013 Subgrants

IF @lsc_gar_form_group_id = 1
BEGIN
	Create table #subgrants
	 (Organization_id int)

	insert #subgrants
	(organization_id)
	  select  distinct o_r.organization_id
		from GRANTEE_PROJECT gp
			JOIN a_WF_PROJECT wfp on gp.wf_project_id = wfp.id
			JOIN a_COMPETITION c on wfp.competition_id = c.id
			JOIN GRANTEE_PROJECT_INFO gpi on gp.grantee_project_id = gpi.grantee_project_id
			JOIN ORGANIZATION o_r on gp.primary_organization_id = o_r.organization_id
		where c.program_id = 3
			--and gpi.duration_start >= @subgrant_duration_start and gpi.duration_end <= @subgrant_duration_end
			and  (gpi.duration_start >= cast(@report_year as varchar(4)) + '-01-01' or gpi.duration_start <= cast(@report_year as varchar(4)) + '-01-01')
			and (gpi.duration_end >= cast(@report_year as varchar(4)) + '-01-01' or gpi.duration_end >= cast(@report_year as varchar(4)) + '-12-31')
	...
	insert #gar
		...
		select 27,
			...
			from a_wf_task wft
				 cross join #grants g
				 JOIN #subgrants s ON g.organization_id = s.organization_id 
			where wft.id = 27
				and g.is_primary_service_area = 1
END --IF @lsc_gar_form_group_id = 1
