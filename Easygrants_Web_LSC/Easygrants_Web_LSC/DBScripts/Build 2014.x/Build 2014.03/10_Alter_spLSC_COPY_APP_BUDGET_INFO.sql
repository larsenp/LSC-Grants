/****** Object:  StoredProcedure [dbo].[spLSC_COPY_APP_BUDGET_INFO]    Script Date: 04/01/2014 11:30:49 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
ALTER PROCEDURE [dbo].[spLSC_COPY_APP_BUDGET_INFO]
...
IF @prev_task_id IN (2,66)
	SET @prev_budget_type_id = 1

IF @prev_task_id IN (4,67)
	SET @prev_budget_type_id = 2
...
IF @new_budget_type_id <> @prev_budget_type_id or @prev_grantee_project_id <> @grantee_project_id
BEGIN
...
BEGIN
	...
	--second level (designates which form is being used)
	UPDATE #bi
		SET new_budget_category_id = bcn.budget_category_id
		FROM #bi
			JOIN a_BUDGET_CATEGORY bcp ON #bi.prev_budget_category_id = bcp.budget_category_id
	...
	--third level (grouping within form)
	UPDATE #bi
		SET new_budget_category_id = bcn.budget_category_id
		FROM #bi
			JOIN a_BUDGET_CATEGORY bcp ON #bi.prev_budget_category_id = bcp.budget_category_id
	...
	--fourth level (actual budget lines) - lsc_fund_code not populated
	UPDATE #bi
		SET new_budget_category_id = bcn.budget_category_id
		FROM #bi
			JOIN a_BUDGET_CATEGORY bcp ON #bi.prev_budget_category_id = bcp.budget_category_id
			...
			JOIN a_BUDGET_CATEGORY bcn ON bcn_p.budget_category_id = bcn.parent_id
				AND bcn.category_type_id = bcp.category_type_id
				AND bcn.sort_order = bcp.sort_order
		WHERE bcn.lsc_fund_code IS NULL
			
	--fourth level (actual budget lines) - lsc_fund_code populated
	UPDATE #bi
		SET new_budget_category_id = bcn.budget_category_id
		FROM #bi
			JOIN a_BUDGET_CATEGORY bcp ON #bi.prev_budget_category_id = bcp.budget_category_id
			...
			JOIN a_BUDGET_CATEGORY bcn ON bcn_p.budget_category_id = bcn.parent_id
				AND bcn.category_type_id = bcp.category_type_id
				AND bcn.lsc_fund_code = bcp.lsc_fund_code
		WHERE bcn.lsc_fund_code IS NOT NULL
