/****** Object:  View [dbo].[vWFTA_COMPETITION_YEAR]    Script Date: 05/08/2014 13:31:28 ******/
SET ANSI_NULLS ON
GO
...
AS
--For NIC and competitive application, TIG LOI and application, and provisional subgrant approval requests, use year of project
select wfta.wf_task_assignment_id, 
	...
	where wfta.wf_task_id in (1, 2, 66, 101)
...
SELECT wfta.wf_task_assignment_id,
     ...
     FROM grantee_project gp
         ...
         JOIN c_time_period tp ON Year(wftas.start_date) = tp.abbr
		 JOIN a_wf_task wft ON wfta.wf_task_id = wft.id
     WHERE c.program_id = 3 and wft.id != 53 and wft.id != 101
