/****** Object:  StoredProcedure [dbo].[spCOPY_EVALUATION_STRUCTURE]    Script Date: 04/28/2009 18:32:10 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[spCOPY_EVALUATION_STRUCTURE]
...
INSERT #c
	...
	sort_order)
	SELECT rc.id,
		rc.lsc_review_performance_area_id,
		rc.abbr,
		rc.description,
		rc.weighting,
		rc.sort_order
		FROM LSC_a_REVIEW_CRITERION rc
			JOIN #pa ON rc.lsc_review_performance_area_id = #pa.old_id
...
BEGIN TRY
	BEGIN TRAN
		...
		UPDATE #c
			...
			FROM #c
				JOIN #pa ON #c.old_pa_id = #pa.old_id
