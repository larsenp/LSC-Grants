/****** Object:  StoredProcedure [dbo].[spLSC_GAR_CSR]    Script Date: 02/03/2009 17:36:15 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[spLSC_GAR_CSR]
...
--Select data
select case_type, 
	sum(a) + sum(b) + sum(c) + sum(d) + sum(e) + sum(f) + sum(g) + sum(h) + sum(i) + sum(j) + sum(k) total,
	sum(a) a, 
	sum(b) b, 
	sum(c) c, 
	sum(d) d, 
...
GRANT EXECUTE ON [dbo].[spLSC_GAR_CSR] TO [LSC.GOV\OIM WebDev]