/****** Object:  StoredProcedure [dbo].[spLSC_GAR_CSR]    Script Date: 03/04/2009 13:48:42 ******/
SET ANSI_NULLS ON
GO
...
BEGIN
	select case_type, 
		sum(case when wfta_year > 2007 then	a + b + f + g + h + ia + ib + ic + k + l
			else a + b + c + d + e + f + g + h + i + j + k end) as total,
		sum(a) A, 
		sum(b) B, 
		sum(c) C, 
		sum(d) D, 
...
BEGIN
	select case_type, 
		sum(a + b + c + d + e + f + g + h + i + j + k ) as total,
		sum(a) A, 
		sum(b) B, 
		sum(c) C, 
		sum(d) D, 
...
BEGIN
	select case_type, 
		sum(a + b + f + g + h + ia + ib + ic + k + l) as total,
		sum(a) A, 
		sum(b) B, 
		sum(f) F, 
		sum(g) G, 
