ALTER VIEW [dbo].[vLSC_a_REVIEW_INQUIRY]
AS
select  i.id,
	'PA' + CAST(pa.sort_order AS varchar(10)) + ', C' + c.abbr + ', Q #' + i.abbr + '. ' + dbo.[fnInquiryPlusSubtopics](i.id) AS inquiry_full_spec,
	CASE WHEN i.description IS NULL THEN dbo.fnSubtopicsByInquiry(i.id,'HTML') ELSE i.description END AS description_or_subtopic
