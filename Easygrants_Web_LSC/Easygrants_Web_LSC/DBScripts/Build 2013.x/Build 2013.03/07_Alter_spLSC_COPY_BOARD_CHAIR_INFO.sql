/****** Object:  StoredProcedure [dbo].[spLSC_COPY_BOARD_CHAIR_INFO]    Script Date: 01/31/2013 09:00:56 ******/
SET ANSI_NULLS ON
GO
...
--Get LSC_BOARD_STRUCTURE id for this organization, year and info source (1607 report or application). If not found, quit (info hasn't been copied forward yet).
SELECT @structure_id = lsc_board_structure_id
	FROM LSC_BOARD_STRUCTURE
	WHERE organization_id = @organization_id
		AND time_period_id = @time_period_id
		AND lsc_board_info_source_id = @lsc_board_info_source_id
...
--If no match, try matching with previous board member info submission (application or 1607 report - NIC does not include board chair info)
BEGIN

	DECLARE @source_time_period_id INT,
		@source_board_info_source_id INT

	IF @lsc_board_info_source_id = 2 --Application: use 1607 if it exists, else previous application
	BEGIN
		SET @source_time_period_id = @prev_time_period_id

		--Check to see if there is a 1607 submission for the previous cycle (i.e., submitted earlier in the same calendar year)
		IF EXISTS
	...
	IF @lsc_board_info_source_id = 3 --1607 report: use previous application (submitted in previous calendar year but in same cycle)
	BEGIN
